...
M_roll_beta_sweep = np.zeros(len(beta_sweep_rad))

for i, beta in enumerate(beta_sweep_rad):
    M_roll_beta_sweep[i] = aero_coeffs.get_roll_moment(beta, p, r, delta_a, delta_r, rho, Va, mav.S, mav.b)
...
M_roll_p_sweep = np.zeros(len(p_sweep_rps))

for i, p in enumerate(p_sweep_rps):
    M_roll_p_sweep[i] = aero_coeffs.get_roll_moment(beta, p, r, delta_a, delta_r, rho, Va, mav.S, mav.b)
...
M_roll_r_sweep = np.zeros(len(r_sweep_rps))

for i, r in enumerate(r_sweep_rps):
    M_roll_r_sweep[i] = aero_coeffs.get_roll_moment(beta, p, r, delta_a, delta_r, rho, Va, mav.S, mav.b)
...
M_roll_delta_a_sweep = np.zeros(len(delta_a_sweep_rad))

for i, delta_a in enumerate(delta_a_sweep_rad):
    M_roll_delta_a_sweep[i] = aero_coeffs.get_roll_moment(beta, p, r, delta_a, delta_r, rho, Va, mav.S, mav.b)
...
M_roll_delta_r_sweep = np.zeros(len(delta_r_sweep_rad))

for i, delta_r in enumerate(delta_r_sweep_rad):
    M_roll_delta_r_sweep[i] = aero_coeffs.get_roll_moment(beta, p, r, delta_a, delta_r, rho, Va, mav.S, mav.b)
...
axs[0,0].plot(beta_sweep_deg, M_roll_beta_sweep)
axs[0,0].grid(True)
axs[0,0].set_xlabel('Beta [deg]')
axs[0,0].set_ylabel(r'$M_y$ [N*m]')
# axs[0].set_title('Roll Moment vs. Beta')

axs[1,0].plot(p_sweep_dps, M_roll_p_sweep)
axs[1,0].grid(True)
axs[1,0].set_xlabel('Roll Rate [deg/s]')
axs[1,0].set_ylabel(r'$M_y$ [N*m]')
# axs[1].set_title('Roll Moment vs. Roll Rate')

axs[1,1].plot(r_sweep_dps, M_roll_r_sweep)
axs[1,1].grid(True)
axs[1,1].set_xlabel('Yaw Rate [deg/s]')
axs[1,1].set_ylabel(r'$M_y$ [N*m]')
# axs[1].set_title('Roll Moment vs. Yaw Rate')

axs[2,0].plot(delta_a_sweep_deg, M_roll_delta_a_sweep)
axs[2,0].grid(True)
axs[2,0].set_xlabel('Aileron Deflection [deg]')
axs[2,0].set_ylabel(r'$M_y$ [N*m]')
# axs[2].set_title('Roll Moment vs. Aileron Deflection')

axs[2,1].plot(delta_r_sweep_deg, M_roll_delta_r_sweep)
axs[2,1].grid(True)
axs[2,1].set_xlabel('Rudder Deflection [deg]')
axs[2,1].set_ylabel(r'$M_y$ [N*m]')
# axs[2].set_title('Roll Moment vs. Rudder Deflection')

plt.suptitle(r'Roll Moment ($l$ or $M_y$) vs. $\beta$, $p$, $r$, $\delta_a$, and $\delta_r$')
